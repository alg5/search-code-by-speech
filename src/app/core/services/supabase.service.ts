...
export class SupabaseService implements OnDestroy {
  ...
  constructor() {
    ...
    const {
      data: { subscription },
    } = this.supabase.auth.onAuthStateChange((event: string, session: AuthSession | null) => {
  ...
  async getProductsBySearch(txtSearch: string, lang: string, maxResults = 20) {
    ...
    const { data, error } = await this.supabase.rpc('search_products_fuzzy', params);
  ...
  async updateProfile(updates: {
    full_name?: string;
    preferences?: any;
  }): Promise<{ role: string; full_name: string; preferences: any } | null> {
    const user = await this.getCurrentUser();
  ...
  async deleteProductCategory(id: number): Promise<void> {
    const { error } = await this.supabase.from('product_categories').delete().eq('id', id);
    if (error) throw error;
  }
  ...
  async deleteProductProcessing(id: number): Promise<void> {
    const { error } = await this.supabase.from('product_processing').delete().eq('id', id);
    if (error) throw error;
  }
  ...
  async getConfigValue(key: string): Promise<string> {
    const { data } = await this.supabase.from('config').select('value').eq('key', key).single();
    return data?.value ?? '';
  }
